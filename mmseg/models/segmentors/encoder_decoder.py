...
class GuidedMix_EncoderDecoder(EncoderDecoder):
    ...
    def __init__(self,
                 ...
                 kl_loss=None,
                 init_cfg=None):
        super(GuidedMix_EncoderDecoder, self).__init__(
                 backbone,
                 decode_head,
                 neck,
                 auxiliary_head,
        ...
        if kl_loss is not None: self.kl_loss = build_loss(kl_loss)
        else: self.kl_loss = None
        
        if mitrans is not None: self.mitrans = builder.build_neck(mitrans)
        else: self.mitrans = None
        
        self.components = components
        self.decode_head_mix = builder.build_head(decode_head)
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, **kwargs):
        ...
        unsup_img = kwargs['unsup.img']
        unsup_img_metas = kwargs['unsup.img_metas']
        unsup_gt_semantic_seg = kwargs['unsup.gt_semantic_seg']
        feats = self.extract_feat(torch.cat([img, unsup_img], 0))
        
        sup_feats = [feat[:len(img)] for feat in feats]
        unsup_feats = [feat[len(img):] for feat in feats]
        
        losses = dict()
        loss_decode = self._decode_head_forward_train(sup_feats, img_metas,
                                                      gt_semantic_seg)
        losses.update(loss_decode)

        # if self.with_auxiliary_head:
        #     loss_aux = self._auxiliary_head_forward_train(
        #         sup_feats, img_metas, gt_semantic_seg)
        #     losses.update(loss_aux)

        unsup_losses = self._second_decode_head_forward_train(img, img_metas, gt_semantic_seg, \
            unsup_img, unsup_img_metas, unsup_gt_semantic_seg, sup_feats, unsup_feats)
        
        unsup_losses = {'unsup.'+k: v for k, v in unsup_losses.items()}
    ...
    def _second_decode_head_forward_train(self, img, img_metas, gt_semantic_seg, \
            unsup_img, unsup_img_metas, unsup_gt_semantic_seg, sup_feats, unsup_feats, alpha=1.0):
        ...
        # seg_logit_unlabeled_main = resize(
        #     input=seg_logit_unlabeled_main,
        #     size=unsup_gt_semantic_seg.shape[2:],
        #     mode='bilinear',
        #     align_corners=self.decode_head.align_corners)
        # seg_logit_unlabeled_auxi = resize(
        #     input=seg_logit_unlabeled_auxi,
        #     size=unsup_gt_semantic_seg.shape[2:],
        #     mode='bilinear',
        #     align_corners=self.auxiliary_head.align_corners)
        
        # pseudo_masks = torch.argmax(seg_logit_main, 1)
        
        ## mixing labeled and unlabeled images
        ...
        seg_logit_unlabeled_firstmain = outputs[0][1]
        
        seg_logit_unlabeled_decoupled_main = seg_logit_secondmain - \
            lam * seg_logit_labeled_firstmain
        seg_logit_labeled_decoupled_main = seg_logit_secondmain - \
            (1-lam) * seg_logit_unlabeled_firstmain[indices]
            
        loss_unlabeled_main = self.kl_loss(seg_logit_unlabeled_decoupled_main, \
            seg_logit_unlabeled_main.detach()[indices])
        loss_labeled_main = self.kl_loss(seg_logit_labeled_decoupled_main, \
            seg_logit_labeled_firstmain.detach())
        # loss_unlabeled_auxi = F.mse_loss(seg_logit_unlabeled_decoupled_auxi, \
        #     seg_logit_unlabeled_auxi.detach()[indices], reduction='mean')
        
        losses = dict()
        # losses['decode.loss_decouple_labeled_ce'] = lam * loss_labeled_main
        losses['decode.loss_decouple_unlabeled_ce'] = loss_unlabeled_main
        
        # losses['aux.loss_ce']=loss_unlabeled_auxi
        return losses